<x-app-layout>
    <section class="section-bordered">
        ...
        <div class="auth-box" style="margin:0 0 60px 0; max-width:100%;">
            ...
            @if ($openExhibitions->isEmpty())
                <p style="color:#FF5555; margin-top:15px;">
                    Tidak ada pameran yang sedang buka saat ini. Kamu belum bisa mengunggah karya —
                    tunggu panitia membuka jadwal pameran berikutnya.
                </p>
            @else
                <form method="POST" action="{{ route('dashboard.artist.store') }}" enctype="multipart/form-data" style="margin-top:20px;">
                    ...
                    <div class="form-group">
                        <label>Pameran (wajib — hanya yang sedang buka)</label>
                        <select name="exhibition_id" required>
                            <option value="">— Pilih pameran —</option>
                            @foreach ($openExhibitions as $ex)
                                <option value="{{ $ex->id }}">{{ $ex->title }} (tutup {{ $ex->end_at->format('d M Y, H:i') }})</option>
                            @endforeach
                        </select>
                        @error('exhibition_id') <p class="form-error">{{ $message }}</p> @enderror
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label>Lelang Berakhir Pada (opsional, kosongkan untuk ikut waktu tutup pameran)</label>
                ...
                <div style="display:flex; justify-content:space-between; align-items:center; padding:20px; background:rgba(13,16,23,0.85); border:1px solid rgba(255,255,255,0.05);">
                    <div>
                        ...
                        <p style="font-size:0.85rem; color:#777; margin-top:4px;">
                            Pameran: {{ $artwork->exhibition?->title ?? '—' }} —
                            Status:
